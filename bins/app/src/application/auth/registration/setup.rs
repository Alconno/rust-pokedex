...
pub fn routes(pg_pool: Arc<Pg>, cfg: &mut web::ServiceConfig) {
    ...
    cfg.route(
        "/auth/register",
        web::post().to(handle_register::<Register<PgRepository, PgService>>)
    );
    cfg.route(
        "/auth/verify-email/resend",
        web::post().to(handle_email_resend::<Register<PgRepository, PgService>>)
    );
    cfg.route(
        "/auth/verify-email/{action_token}",
        web::get().to(handle_verify_email::<Register<PgRepository, PgService>>)
    );
