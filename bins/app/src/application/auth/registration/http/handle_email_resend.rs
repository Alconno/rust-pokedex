use crate::application::auth::registration::{contract::RegisterContract, data::UserValidationRequestData};
use actix_web::{http::header, web, HttpRequest, HttpResponse};
use error::Error;
use support::helpers::base_url;
use validr::Validation;

pub async fn handle_email_resend<T: RegisterContract>(
    service: web::Data<T>,
    data: web::Json<UserValidationRequestData>,
    request: HttpRequest,
) -> Result<HttpResponse, Error> {
    let base_url = base_url::get_base_url(&request);
    let data = data.into_inner().validate()?.insertable();
    let action_token = service.resend_action_token(data, &base_url).await?;
